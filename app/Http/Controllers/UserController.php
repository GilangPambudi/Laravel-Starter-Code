...
class UserController extends Controller
{
    ...
    public function store_ajax(Request $request)
    {
        ...
        if ($request->ajax() || $request->wantsJson()) {
            $rules = [
                'level_id' => ['required', 'integer', 'exists:m_level,level_id'], // Removed extra space
                'username' => ['required', 'max:20', 'unique:m_user,username'], // Removed extra space
                'nama' => ['required', 'max:100'], // Removed extra space
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'password' => 'required|string|min:5', //password harus diisi, berupa string dan minimal 5 karakter
    ...
    public function update(Request $request, string $id)
    {
        $request->validate([
            ...
            //dan bernilai unik di table m_user kolom username kecuali untuk user dengan id yang sedang diubah
            'username' => 'required|string|min:3|unique:m_user,username,' . $id . ',user_id',
            'nama' => 'required|string|max:100', //nama harus diisi, berupa string dan maksimal 100 karakter
            'password' => 'nullable|string|min:5', //password boleh diisi atau tidak, berupa string dan minimal 5 karakter
            'level_id' => 'required|integer' //level_id harus diisi, berupa integer
        ]);
    ...
    {
        $user = UserModel::find($id);
        return view('user.confirm_ajax')->with('user', $user);
    }
    ...
    public function export_excel()
    {
        ...
        foreach ($user as $data) {
            $sheet->setCellValue('A' . $baris, $data->username);
            $sheet->setCellValue('B' . $baris, $data->nama);
            $sheet->setCellValue('C' . $baris, $data->level->level_nama);
            $no++;
            $baris++;
        }
